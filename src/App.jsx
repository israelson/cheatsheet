...
const CATEGORIES = [
  ...
  "Linux PrivEsc","Linux Admin","Linux Server",
  "Windows PrivEsc","Windows Admin","Windows Server",
  ...
  "Terraform","Ansible","Kubernetes","Docker / Compose","Wireshark / TShark","Misc / Snippets","Custom"
];
...
const INITIAL_COMMANDS = [
  ...
  // Linux Admin
  {id:125,title:"Informações do Sistema",category:"Linux Admin",tags:["medium"],command:"uname -a && lsb_release -a 2>/dev/null || cat /etc/os-release",desc:"Exibe kernel, distro e versão do sistema"},
  {id:126,title:"Uso de CPU e Memória",category:"Linux Admin",tags:["medium"],command:"top -bn1 | head -20",desc:"Snapshot de CPU e memória sem interface interativa"},
  {id:127,title:"Uso de Disco",category:"Linux Admin",tags:["medium"],command:"df -hT && du -sh /*/  2>/dev/null | sort -rh | head -15",desc:"Espaço em disco por partição e diretórios maiores"},
  {id:128,title:"Processos por CPU",category:"Linux Admin",tags:["medium"],command:"ps aux --sort=-%cpu | head -15",desc:"Top 15 processos por consumo de CPU"},
  {id:129,title:"Portas em Escuta",category:"Linux Admin",tags:["medium"],command:"ss -tlnp",desc:"Lista portas TCP em escuta com PID do processo"},
  {id:130,title:"Todas Conexões de Rede",category:"Linux Admin",tags:["medium"],command:"ss -tunap",desc:"Conexões TCP e UDP ativas com processos associados"},
  {id:131,title:"Usuários Logados",category:"Linux Admin",tags:["medium"],command:"w && last -n 20",desc:"Usuários conectados agora e últimos 20 logins"},
  {id:132,title:"Listar Usuários do Sistema",category:"Linux Admin",tags:["medium"],command:"getent passwd | awk -F: '$3>=1000 {print $1,$3,$6,$7}' | column -t",desc:"Lista usuários reais (UID>=1000) com home e shell"},
  {id:133,title:"Criar Usuário com Home",category:"Linux Admin",tags:["high"],command:"useradd -m -s /bin/bash {USER} && echo '{USER}:{PASS}' | chpasswd",desc:"Cria usuário com diretório home e define senha"},
  {id:134,title:"Adicionar ao Sudoers",category:"Linux Admin",tags:["high"],command:"usermod -aG sudo {USER}",desc:"Adiciona usuário ao grupo sudo (Debian/Ubuntu) ou wheel (RHEL)"},
  {id:135,title:"Permissões de Arquivo",category:"Linux Admin",tags:["medium"],command:"chmod 750 /caminho && chown {USER}:{USER} /caminho",desc:"Define permissões e dono de arquivo ou diretório"},
  {id:136,title:"Encontrar Arquivos Grandes",category:"Linux Admin",tags:["medium"],command:"find / -type f -size +100M -exec ls -lh {} \\; 2>/dev/null | sort -rh | head -20",desc:"Localiza os 20 maiores arquivos no sistema"},
  {id:137,title:"Logs do Sistema",category:"Linux Admin",tags:["medium"],command:"journalctl -xe --since '1 hour ago' | tail -100",desc:"Últimas entradas de log do systemd na última hora"},
  {id:138,title:"Agendar Tarefa (Cron)",category:"Linux Admin",tags:["medium"],command:"(crontab -l 2>/dev/null; echo '0 3 * * * /opt/backup.sh >> /var/log/backup.log 2>&1') | crontab -",desc:"Adiciona cron job sem sobrescrever os existentes"},
  {id:139,title:"Verificar Serviço",category:"Linux Admin",tags:["medium"],command:"systemctl status {USER} && journalctl -u {USER} -n 50 --no-pager",desc:"Status e últimas 50 linhas de log de um serviço"},
  {id:140,title:"Reiniciar Serviço",category:"Linux Admin",tags:["medium"],command:"systemctl restart {USER} && systemctl status {USER}",desc:"Reinicia um serviço e verifica o resultado"},
  {id:141,title:"Rotas de Rede",category:"Linux Admin",tags:["medium"],command:"ip route show && ip -6 route show",desc:"Tabelas de roteamento IPv4 e IPv6"},
  {id:142,title:"Interfaces de Rede",category:"Linux Admin",tags:["medium"],command:"ip -br addr show",desc:"Lista interfaces com IPs de forma compacta"},
  {id:143,title:"Verificar Abertura de Porta Remota",category:"Linux Admin",tags:["medium"],command:"nc -zv {RHOST} {RPORT} 2>&1 || timeout 3 bash -c 'echo >/dev/tcp/{RHOST}/{RPORT}' && echo open",desc:"Testa conectividade TCP em porta específica sem telnet"},
  {id:144,title:"Capturar Tráfego",category:"Linux Admin",tags:["high"],command:"tcpdump -i any -nn -w /tmp/cap.pcap host {RHOST} &",desc:"Captura pacotes de/para um host em background"},
  {id:145,title:"Atualizar Sistema (Debian/Ubuntu)",category:"Linux Admin",tags:["medium"],command:"apt update && apt upgrade -y && apt autoremove -y",desc:"Atualiza todos os pacotes e remove os desnecessários"},
  {id:146,title:"Atualizar Sistema (RHEL/Fedora)",category:"Linux Admin",tags:["medium"],command:"dnf update -y && dnf autoremove -y",desc:"Atualiza pacotes e remove dependências órfãs"},
  // Linux Server
  {id:147,title:"Status do Nginx",category:"Linux Server",tags:["medium"],command:"systemctl status nginx && nginx -t",desc:"Verifica status e valida configuração do Nginx"},
  {id:148,title:"Recarregar Nginx",category:"Linux Server",tags:["medium"],command:"nginx -t && systemctl reload nginx",desc:"Valida config e recarrega Nginx sem downtime"},
  {id:149,title:"Logs de Acesso Nginx",category:"Linux Server",tags:["medium"],command:"tail -f /var/log/nginx/access.log | awk '{print $1, $7, $9}'",desc:"Logs de acesso em tempo real: IP, URI, status"},
  {id:150,title:"Status do Apache",category:"Linux Server",tags:["medium"],command:"systemctl status apache2 || systemctl status httpd && apachectl -t",desc:"Status e validação de config do Apache (Debian/RHEL)"},
  {id:151,title:"VirtualHosts Apache Ativos",category:"Linux Server",tags:["medium"],command:"apachectl -S 2>&1 | head -40",desc:"Lista todos os VirtualHosts configurados e porta de escuta"},
  {id:152,title:"Certificado SSL - Validade",category:"Linux Server",tags:["high"],command:"echo | openssl s_client -connect {RHOST}:443 -servername {RHOST} 2>/dev/null | openssl x509 -noout -dates -subject",desc:"Verifica validade e domínio do certificado SSL"},
  {id:153,title:"Gerar Cert Let's Encrypt",category:"Linux Server",tags:["high"],command:"certbot --nginx -d {DOMAIN} --non-interactive --agree-tos -m admin@{DOMAIN}",desc:"Emite e instala certificado Let's Encrypt via Certbot no Nginx"},
  {id:154,title:"Renovar Certs Let's Encrypt",category:"Linux Server",tags:["medium"],command:"certbot renew --dry-run && certbot renew",desc:"Testa e efetua renovação de todos os certificados gerenciados"},
  {id:155,title:"MySQL - Status e Versão",category:"Linux Server",tags:["medium"],command:"mysql -u root -p -e 'STATUS; SHOW DATABASES;'",desc:"Conecta ao MySQL e lista bancos de dados"},
  {id:156,title:"MySQL - Backup de Database",category:"Linux Server",tags:["high"],command:"mysqldump -u root -p --single-transaction --routines --triggers {USER} | gzip > /backup/{USER}_$(date +%F).sql.gz",desc:"Dump completo de uma database com compressão"},
  {id:157,title:"PostgreSQL - Status",category:"Linux Server",tags:["medium"],command:"systemctl status postgresql && psql -U postgres -c '\\l'",desc:"Status do serviço e lista de databases PostgreSQL"},
  {id:158,title:"PostgreSQL - Backup",category:"Linux Server",tags:["high"],command:"pg_dump -U postgres -Fc {USER} > /backup/{USER}_$(date +%F).dump",desc:"Backup em formato custom (restaurável com pg_restore)"},
  {id:159,title:"Docker - Containers Ativos",category:"Linux Server",tags:["medium"],command:"docker ps --format 'table {{.Names}}\\t{{.Status}}\\t{{.Ports}}'",desc:"Lista containers em execução com nome, status e portas"},
  {id:160,title:"Docker - Logs em Tempo Real",category:"Linux Server",tags:["medium"],command:"docker logs -f --tail 100 {USER}",desc:"Segue os últimos 100 logs de um container"},
  {id:161,title:"Docker - Limpeza Geral",category:"Linux Server",tags:["high"],command:"docker system prune -af --volumes",desc:"Remove containers parados, imagens não usadas e volumes órfãos"},
  {id:162,title:"Firewall UFW - Regras",category:"Linux Server",tags:["medium"],command:"ufw status numbered",desc:"Lista regras UFW numeradas (Ubuntu/Debian)"},
  {id:163,title:"Firewall UFW - Liberar Porta",category:"Linux Server",tags:["medium"],command:"ufw allow {RPORT}/tcp && ufw reload",desc:"Libera uma porta TCP no UFW e recarrega as regras"},
  {id:164,title:"Firewall firewalld - Regras",category:"Linux Server",tags:["medium"],command:"firewall-cmd --list-all",desc:"Lista regras ativas no firewalld (RHEL/Fedora/CentOS)"},
  {id:165,title:"Firewall firewalld - Liberar Porta",category:"Linux Server",tags:["medium"],command:"firewall-cmd --permanent --add-port={RPORT}/tcp && firewall-cmd --reload",desc:"Adiciona porta permanentemente ao firewalld"},
  {id:166,title:"SSH - Hardening Básico",category:"Linux Server",tags:["high"],command:"sed -i 's/#PermitRootLogin yes/PermitRootLogin no/;s/#PasswordAuthentication yes/PasswordAuthentication no/' /etc/ssh/sshd_config && systemctl restart sshd",desc:"Desabilita login root e autenticação por senha no SSH"},
  {id:167,title:"Verificar Atualizações de Segurança",category:"Linux Server",tags:["high"],command:"apt list --upgradable 2>/dev/null | grep -i security || dnf updateinfo list security 2>/dev/null",desc:"Lista apenas atualizações de segurança pendentes"},
  {id:168,title:"Carga do Sistema (Load Average)",category:"Linux Server",tags:["medium"],command:"uptime && cat /proc/loadavg && nproc",desc:"Exibe uptime, load average e número de CPUs"},
  // Windows Admin
  ...
  // Terraform
  {id:300,title:"Init",category:"Terraform",tags:["medium"],command:"terraform init",desc:"Inicializa o diretório e baixa providers/modules"},
  {id:301,title:"Plan",category:"Terraform",tags:["high"],command:"terraform plan -out=tfplan",desc:"Gera e salva o plano de execução sem aplicar"},
  {id:302,title:"Apply",category:"Terraform",tags:["high"],command:"terraform apply tfplan",desc:"Aplica o plano salvo anteriormente"},
  {id:303,title:"Apply Auto-Approve",category:"Terraform",tags:["high"],command:"terraform apply -auto-approve",desc:"Aplica mudanças sem pedir confirmação"},
  {id:304,title:"Destroy",category:"Terraform",tags:["critical"],command:"terraform destroy -auto-approve",desc:"Destrói toda a infraestrutura gerenciada"},
  {id:305,title:"Validate",category:"Terraform",tags:["medium"],command:"terraform validate",desc:"Valida sintaxe e configuração dos arquivos .tf"},
  {id:306,title:"Format",category:"Terraform",tags:["medium"],command:"terraform fmt -recursive",desc:"Formata todos os arquivos .tf no padrão canônico"},
  {id:307,title:"State List",category:"Terraform",tags:["medium"],command:"terraform state list",desc:"Lista todos os recursos no state atual"},
  {id:308,title:"State Show",category:"Terraform",tags:["medium"],command:"terraform state show {USER}",desc:"Exibe detalhes de um recurso no state (ex: aws_instance.web)"},
  {id:309,title:"State Move",category:"Terraform",tags:["high"],command:"terraform state mv {USER} {PASS}",desc:"Move recurso de um endereço para outro no state"},
  {id:310,title:"State Remove",category:"Terraform",tags:["high"],command:"terraform state rm {USER}",desc:"Remove recurso do state sem destruir na nuvem"},
  {id:311,title:"Import",category:"Terraform",tags:["high"],command:"terraform import {USER} {PASS}",desc:"Importa recurso existente para o state (ex: aws_instance.web i-1234)"},
  {id:312,title:"Output",category:"Terraform",tags:["medium"],command:"terraform output -json",desc:"Exibe todos os outputs em formato JSON"},
  {id:313,title:"Workspace List / Select",category:"Terraform",tags:["medium"],command:"terraform workspace list && terraform workspace select {USER}",desc:"Lista workspaces e seleciona um"},
  {id:314,title:"Workspace New",category:"Terraform",tags:["medium"],command:"terraform workspace new {USER}",desc:"Cria e entra em um novo workspace"},
  {id:315,title:"Refresh",category:"Terraform",tags:["medium"],command:"terraform refresh",desc:"Sincroniza o state com o estado real da infraestrutura"},
  {id:316,title:"Graph",category:"Terraform",tags:["medium"],command:"terraform graph | dot -Tpng > graph.png",desc:"Gera grafo visual de dependências dos recursos"},
  // Ansible
  {id:320,title:"Ping Todos os Hosts",category:"Ansible",tags:["medium"],command:"ansible all -i inventory.yml -m ping",desc:"Testa conectividade SSH com todos os hosts do inventário"},
  {id:321,title:"Executar Playbook",category:"Ansible",tags:["high"],command:"ansible-playbook -i inventory.yml playbook.yml",desc:"Executa um playbook com o inventário especificado"},
  {id:322,title:"Dry-Run (Check Mode)",category:"Ansible",tags:["medium"],command:"ansible-playbook -i inventory.yml playbook.yml --check --diff",desc:"Simula execução mostrando o que seria alterado"},
  {id:323,title:"Executar com Tags",category:"Ansible",tags:["medium"],command:"ansible-playbook -i inventory.yml playbook.yml --tags {USER}",desc:"Executa apenas tasks com a tag especificada"},
  {id:324,title:"Limitar a Hosts",category:"Ansible",tags:["medium"],command:"ansible-playbook -i inventory.yml playbook.yml --limit {RHOST}",desc:"Executa o playbook apenas em hosts específicos"},
  {id:325,title:"Ad-hoc — Comando Shell",category:"Ansible",tags:["medium"],command:"ansible all -i inventory.yml -m shell -a 'uptime' -u {USER}",desc:"Executa comando shell em todos os hosts"},
  {id:326,title:"Ad-hoc — Copiar Arquivo",category:"Ansible",tags:["medium"],command:"ansible all -i inventory.yml -m copy -a 'src=/tmp/file dest=/tmp/file'",desc:"Copia arquivo local para todos os hosts remotos"},
  {id:327,title:"Ad-hoc — Gerenciar Serviço",category:"Ansible",tags:["medium"],command:"ansible all -i inventory.yml -m service -a 'name=nginx state=restarted' --become",desc:"Reinicia serviço em todos os hosts com sudo"},
  {id:328,title:"Vault — Encriptar Arquivo",category:"Ansible",tags:["high"],command:"ansible-vault encrypt {USER}",desc:"Encripta arquivo de variáveis com Ansible Vault"},
  {id:329,title:"Vault — Editar / Ver",category:"Ansible",tags:["high"],command:"ansible-vault edit {USER} || ansible-vault view {USER}",desc:"Edita ou visualiza arquivo encriptado pelo Vault"},
  {id:330,title:"Galaxy — Instalar Role",category:"Ansible",tags:["medium"],command:"ansible-galaxy install {USER} && ansible-galaxy list",desc:"Instala role do Ansible Galaxy e lista as instaladas"},
  {id:331,title:"Listar Tasks do Playbook",category:"Ansible",tags:["medium"],command:"ansible-playbook playbook.yml --list-tasks --list-hosts",desc:"Lista todas as tasks e hosts sem executar"},
  {id:332,title:"Inventário Estruturado",category:"Ansible",tags:["medium"],command:"ansible-inventory -i inventory.yml --list --yaml",desc:"Exibe o inventário em formato YAML"},
  {id:333,title:"Instalar Requirements",category:"Ansible",tags:["medium"],command:"ansible-galaxy install -r requirements.yml",desc:"Instala todas as roles declaradas em requirements.yml"},
  // Kubernetes
  {id:340,title:"Get Pods (todos namespaces)",category:"Kubernetes",tags:["medium"],command:"kubectl get pods -A -o wide",desc:"Lista todos os pods com node e IP"},
  {id:341,title:"Get Recursos do Namespace",category:"Kubernetes",tags:["medium"],command:"kubectl get deploy,svc,ing,cm,secret -n {USER}",desc:"Lista recursos principais de um namespace"},
  {id:342,title:"Apply Manifesto",category:"Kubernetes",tags:["high"],command:"kubectl apply -f {USER} --dry-run=client && kubectl apply -f {USER}",desc:"Valida e aplica manifesto YAML"},
  {id:343,title:"Delete Recurso",category:"Kubernetes",tags:["high"],command:"kubectl delete -f {USER}",desc:"Remove recursos definidos em um manifesto"},
  {id:344,title:"Logs de Pod",category:"Kubernetes",tags:["medium"],command:"kubectl logs -f {USER} -n {DOMAIN} --tail=100",desc:"Segue logs de um pod em tempo real"},
  {id:345,title:"Exec em Pod",category:"Kubernetes",tags:["high"],command:"kubectl exec -it {USER} -n {DOMAIN} -- /bin/bash",desc:"Abre shell interativo em um pod"},
  {id:346,title:"Describe Recurso",category:"Kubernetes",tags:["medium"],command:"kubectl describe pod {USER} -n {DOMAIN}",desc:"Exibe detalhes e eventos de um recurso"},
  {id:347,title:"Port-Forward",category:"Kubernetes",tags:["medium"],command:"kubectl port-forward svc/{USER} {LPORT}:{RPORT} -n {DOMAIN}",desc:"Encaminha porta local para um service no cluster"},
  {id:348,title:"Rollout Status",category:"Kubernetes",tags:["medium"],command:"kubectl rollout status deploy/{USER} -n {DOMAIN}",desc:"Acompanha o progresso de um rollout"},
  {id:349,title:"Rollback",category:"Kubernetes",tags:["high"],command:"kubectl rollout undo deploy/{USER} -n {DOMAIN}",desc:"Reverte deployment para a revisão anterior"},
  {id:350,title:"Scale Deployment",category:"Kubernetes",tags:["high"],command:"kubectl scale deploy/{USER} --replicas={RPORT} -n {DOMAIN}",desc:"Escala o número de réplicas de um deployment"},
  {id:351,title:"Top Nodes e Pods",category:"Kubernetes",tags:["medium"],command:"kubectl top nodes && kubectl top pods -A --sort-by=memory",desc:"Exibe uso de CPU e memória de nodes e pods"},
  {id:352,title:"Contextos (Clusters)",category:"Kubernetes",tags:["medium"],command:"kubectl config get-contexts && kubectl config use-context {USER}",desc:"Lista e troca contexto de cluster"},
  {id:353,title:"Criar Secret",category:"Kubernetes",tags:["high"],command:"kubectl create secret generic {USER} --from-literal=key={PASS} -n {DOMAIN}",desc:"Cria Secret genérico no namespace"},
  {id:354,title:"Copiar Arquivo do Pod",category:"Kubernetes",tags:["medium"],command:"kubectl cp {DOMAIN}/{USER}:/caminho /tmp/arquivo",desc:"Copia arquivo de dentro de um pod para local"},
  {id:355,title:"Eventos do Cluster",category:"Kubernetes",tags:["medium"],command:"kubectl get events -A --sort-by='.lastTimestamp' | tail -30",desc:"Últimos 30 eventos ordenados por data"},
  // Docker / Compose
  {id:360,title:"Build de Imagem",category:"Docker / Compose",tags:["medium"],command:"docker build -t {USER}:{PASS} -f Dockerfile .",desc:"Constrói imagem com tag a partir de Dockerfile"},
  {id:361,title:"Run Interativo",category:"Docker / Compose",tags:["medium"],command:"docker run -it --rm -p {LPORT}:{RPORT} --name {USER} {PASS}",desc:"Inicia container interativo com port mapping"},
  {id:362,title:"Run em Background",category:"Docker / Compose",tags:["medium"],command:"docker run -d --restart unless-stopped -p {LPORT}:{RPORT} --name {USER} {PASS}",desc:"Inicia container em background com restart automático"},
  {id:363,title:"Exec em Container",category:"Docker / Compose",tags:["medium"],command:"docker exec -it {USER} /bin/bash",desc:"Abre shell em container em execução"},
  {id:364,title:"Stats em Tempo Real",category:"Docker / Compose",tags:["medium"],command:"docker stats --format 'table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}'",desc:"Monitora CPU, memória e rede de todos os containers"},
  {id:365,title:"Inspecionar Container",category:"Docker / Compose",tags:["medium"],command:"docker inspect {USER} | python3 -m json.tool",desc:"Exibe configuração completa do container em JSON"},
  {id:366,title:"Compose Up",category:"Docker / Compose",tags:["medium"],command:"docker compose up -d --build",desc:"Sobe todos os serviços em background com rebuild"},
  {id:367,title:"Compose Down",category:"Docker / Compose",tags:["high"],command:"docker compose down -v",desc:"Para e remove containers e volumes do compose"},
  {id:368,title:"Compose Logs",category:"Docker / Compose",tags:["medium"],command:"docker compose logs -f --tail=50 {USER}",desc:"Segue logs de um serviço específico do compose"},
  {id:369,title:"Network — Listar",category:"Docker / Compose",tags:["medium"],command:"docker network ls && docker network inspect {USER}",desc:"Lista redes e inspeciona uma rede específica"},
  {id:370,title:"Volume — Listar",category:"Docker / Compose",tags:["medium"],command:"docker volume ls && docker volume inspect {USER}",desc:"Lista volumes e exibe detalhes de montagem"},
  {id:371,title:"Limpeza Geral",category:"Docker / Compose",tags:["high"],command:"docker system prune -af --volumes && docker builder prune -af",desc:"Remove containers, imagens, volumes e cache de build"},
  {id:372,title:"Push para Registry",category:"Docker / Compose",tags:["high"],command:"docker tag {USER}:{PASS} {RHOST}/{USER}:{PASS} && docker push {RHOST}/{USER}:{PASS}",desc:"Tag e push de imagem para registry privado"},
  {id:373,title:"Save e Load de Imagem",category:"Docker / Compose",tags:["medium"],command:"docker save {USER}:{PASS} | gzip > /tmp/image.tar.gz && docker load < /tmp/image.tar.gz",desc:"Exporta imagem para arquivo e importa em outro host"},
  // Wireshark / TShark
  ...
  {id:207,title:"[TSHARK] Top Protocolos",category:"Wireshark / TShark",tags:["medium"],command:"tshark -r /tmp/cap.pcap -q -z io,phs",desc:"Hierarquia de protocolos capturados"},
